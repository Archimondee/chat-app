...
export interface AbortWretch {
   ...
   */
  signal: <T extends AbortWretch, C, R>(
    this: T & Wretch<T, C, R>,
...
export interface AbortResolver {
   ...
   * @param controller - An instance of AbortController
   */
  setTimeout: <T, C extends AbortResolver, R>(
   ...
   * ```
   */
  controller: <T, C extends AbortResolver, R>(
    this: C & WretchResponseChain<T, C, R>,
  ) => [any, this];
  /**
 ...
 * ```
 */
declare const abort: () => WretchAddon<AbortWretch, AbortResolver>;
